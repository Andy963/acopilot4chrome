...
- Capture actions expose their guidance on hover, and the header provides a clear-conversation action that removes the visible history and excludes it from subsequent requests.
- Captured context is attached to the question you send it with: it appears at the top of that message tagged with a colored page/selection icon (matching the capture buttons), and the staging area clears afterward, so each question owns its own context.
...
- Copy any message from its footer (assistant replies copy their raw Markdown), regenerate a question's answer in place with the regenerate button, and copy any code block from the button that appears when you hover it.
